...
import mapper.BookphotoMapper;
import mapper.PurchaseMapper;
import mapper.SaddbMapper;
import mapper.UseraddrMapper;
...
public class UserService {
	...
	private BookphotoMapper bookphotoMapper;
	private PurchaseMapper purchaseMapper;
	private SaddbMapper saddbMapper;
	...
	@Autowired
	public UserService(UserloginMapper userloginMapper, UserinfoMapper userinfoMapper, UseraddrMapper useraddrMapper,
			BookinfoMapper bookinfoMapper, BookphotoMapper bookphotoMapper, PurchaseMapper purchaseMapper,
			SaddbMapper saddbMapper) {
		super();
		this.userloginMapper = userloginMapper;
		this.userinfoMapper = userinfoMapper;
		this.useraddrMapper = useraddrMapper;
		this.bookinfoMapper = bookinfoMapper;
		this.bookphotoMapper = bookphotoMapper;
		this.purchaseMapper = purchaseMapper;
		this.saddbMapper = saddbMapper;
	}
	...
	public Userinfo getUserinfo(String uphone) {
		Userinfo userinfo = userinfoMapper.getUserinfo(uphone);
		if (userinfo != null)
			return userinfo;
		else
			return null;
	 ...
	 * @return：是否登录成功
	 */
	public boolean loginCheck(String uphone, String upassword) {
		if (userloginMapper.getUserloginByUphone(uphone).getUpassword().equals(upassword))
			return true;
		else
			return false;
	}
	...
	public boolean isUphoneExit(String uphone) {
		if (userloginMapper.getUserloginByUphone(uphone) != null)
			return true;
		else
			return false;
	}
	 ...
	 */
	@Transactional
	public boolean addUserinfo(Userinfo userinfo, Userlogin userlogin) {
		int cnt = userinfoMapper.addUserinfo(userinfo);
		if (cnt > 0) {
			int cnt2 = userloginMapper.addUserlogin(userlogin);
			if (cnt2 > 0)
				return true;
			else
				return false;
	...
	public boolean updateUserinfo(Userinfo userinfo) {
		int cnt = userinfoMapper.updateUserinfo(userinfo);
		if (cnt > 0)
			return true;
		else
			return false;
